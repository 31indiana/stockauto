...
myToken = "mytokenmytokenmytokenmytokenmytokenmytokenmytokenmytokenmytokenmytokenmytokenmytokenmytokenmytoken"
def dbgout(message):
    """인자로 받은 문자열을 파이썬 셸과 슬랙으로 동시에 출력한다."""
    print(datetime.now().strftime('[%m/%d %H:%M:%S]'), message)
    strbuf = datetime.now().strftime('[%m/%d %H:%M:%S] ') + message
    post_message(myToken,"#autotrading", strbuf)
...
def get_target_price(code):
    ...
    try:
        ...
        target_price = today_open + (lastday_high - lastday_low) * 0.5   #k:계수 0.5
        return target_price
...
if __name__ == '__main__': 
    try:
        symbol_list = ['A226490','A114800','A229200','A251340','A261220','A271050']   #자동매매 원하는 종목티커 
        ...
        #A261220원유(kodex)            #A271050원유인버스(kodex) 
        #A204480차이나csi300레버리지(tiger)
        #A200250인도nifty50(kosef) 
        #Rquant추천종목   
        #미국 #동남아   #ETN  
        bought_list = []     # 매수 완료된 종목 리스트
        target_buy_count = 3 # 매수할 최대 종목 수
        buy_percent = 0.33   #종목별 분산투자 비율
